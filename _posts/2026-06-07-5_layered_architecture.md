...
# 1. 아키텍처의 패러다임 전환과 육각형 원류 (Architectural Evolution & Foundations)
...
## 1.2. 아키텍처의 원류: 헥사고날 아키텍처(Hexagonal Architecture)의 영감
...
## 2. 디자인 패턴 기반의 5대 레이어 심층 명세 (Deep Dive into the 5-Layers)
...
### 2.1. 실행 레이어 (Execution Layer: `main.py`)
...
### 2.2. 파이프라인 레이어 (Pipeline Layer: `src/pipeline/`)
...
### 2.3. 오케스트레이터 레이어 (Orchestrator Layer: `***_service.py`)
...
### 2.4. 팩토리 레이어 (Factory Layer: `***_factory.py`)
...
### 2.5. 세부 태스크 레이어 (Detailed Task Layer: `***.py` & `abstract_***.py`)
...
# 3. 데이터 및 제어 흐름의 공학적 메커니즘 (Data & Control Flow)

## 3.1. 포트 앤 어댑터 메커니즘의 데이터 도메인 구현 검증
